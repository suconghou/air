...
var app = {
	serve: function(cfg) {
		...
		var server = instance
			.listen(instance.get('port'), function() {
		...
		instance.get('/reload', function(req, res, next) {
			...
			if (cfg.watch) {
				...
				reload =
					'var a=new XMLHttpRequest;a.timeout=0;a.open("POST","' +
					url +
					'",1),a.onreadystatechange=function(){4==a.readyState&&location.reload()},a.send();';
			}
		...
		instance.use(
			express.static(cfg.workPath, {
				maxAge: cfg.debug ? '5s' : '1h',
	...
	route: function(instance) {
		...
		instance.use(/[\w\-\/]+\.css$/, function(req, res, next) {
			...
			var files = req.baseUrl
				...
				.map(function(item) {
					return path.join(
						req.headers.serveDir ? req.headers.serveDir : cfg.workPath,
						item.replace(/externals./, exterPath) + '.less'
					);
				});
	...
	compress: function(args, cfg) {
		var watchCompress = function(cfg) {
			...
			if (cfg.watch && !env.watched) {
				...
				cfg.waitChange.on('compress', function(file) {
					...
					childArgv = childArgv.filter(function(item) {
		...
		if (args.length <= 1) {
			compress.compressByConfig(
				cfg,
				function(files, savename) {
					m.log(files.join('\r\n') + '\r\nsave as:' + savename);
				},
				function(code, errorMsg) {
			...
			var jsfiles = args
				.unique()
				.filter(function(item) {
					return item && item.substr(-3) == '.js';
				})
				.map(function(item) {
					return path.resolve(cfg.workPath, item);
				});
			var lessfiles = args
				.unique()
				.filter(function(item) {
					return item && item.substr(-5) == '.less';
				})
				.map(function(item) {
					return path.resolve(cfg.workPath, item);
				});
			var imgfiles = args
				.unique()
				.filter(function(item) {
					return /^.+\.(png|jpg|jpeg)$/i.test(item);
				})
				.map(function(item) {
					return path.resolve(cfg.workPath, item);
				});
					...
					function(content) {
						...
						var savename =
							path.join(
								...
								lessfiles
									.map(function(item) {
										return path.basename(item, '.less');
									})
									.join('-')
							) + ext;
						fs.writeFile(savename, content.content, function(err) {
	...
	build: function(args, cfg) {
		...
		if (args.length > 1) {
			...
			if (pkg.entry) {
				...
				entry = entry
					...
					.unique()
					.map(function(item) {
						return path.resolve(cfg.cfgPath, item);
					});
			} else {
		...
		var options = {
			module: {
				loaders: [
					...
					{ test: /\.(html|tpl)$/, loader: 'html' },
					{
						...
						loader: 'url?limit=' + (parseInt(pkg.dataUrlLimit) ? parseInt(pkg.dataUrlLimit) : 8192),
					},
			...
			plugins: [
				new ExtractTextPlugin(cfg.cfgPath ? path.join(cfg.cfgPath, '[name].min.css') : './[name].min.css', {
					allChunks: true,
				}),
			],
			entry: { app: entry },
		...
		if (cfg.watch && cfg.debug) {
			...
			options.entry.app.unshift(
				'webpack-dev-server/client?http://localhost:' + cfg.port + '/',
			...
			if (cfg.optimization) {
				options.output.publicPath = cfg.publicPath
					? cfg.publicPath
					: pkg.publicPath
					? pkg.publicPath
					: '/dist/';
	...
	compressLess: function(lessfiles, cfg, successCallback, errorCallback) {
		this.getMaxUpdateTime(lessfiles, errorCallback, function(updateTime, key) {
			...
			if (content.updateTime >= updateTime && content.ver == cfg.ver) {
				...
				var includePath = path.isAbsolute(cfg.lessLibPath)
					? cfg.lessLibPath
					: [
							path.join(cfg.workPath, cfg.lessLibPath),
							path.join(path.dirname(cfg.workPath), cfg.lessLibPath),
							path.join(path.dirname(path.dirname(cfg.workPath)), cfg.lessLibPath),
							path.join(path.dirname(path.dirname(path.dirname(cfg.workPath))), cfg.lessLibPath),
					  ];
				...
				var option = {
					...
					paths: includePath,
					urlArgs: cfg.ver ? 'ver=' + cfg.ver : null,
				};
				if (!cfg.debug) {
	...
	getRequestFiles: function(cfg, req, callback) {
		...
		fs.exists(configFile, function(exists) {
			...
			if (exists) {
				try {
					...
					if (config.static && config.static[params.ext]) {
						hotPath = Object.keys(config.static[params.ext])
							.filter(function(item) {
								return item;
							})
							.unique();
			...
			if (hotPath.indexOf(params.file) >= 0) {
				files = config.static[params.ext][params.file]
					.filter(function(item) {
						return item;
					})
					.unique()
					.map(function(item) {
						return path.resolve(basePath, item.replace(/externals./, exterPath));
					});
			} else {
				files = params.file
					.split('-')
					...
					.map(function(item) {
						return path.resolve(
							basePath,
							item.replace(/externals./, exterPath) + '.' + (params.ext == 'js' ? params.ext : 'less')
						);
					});
			}
			callback(files);
			...
			try {
				...
				if (config.static) {
					if (config.static.css) {
						hotPathLess = Object.keys(config.static.css)
							.filter(function(item) {
								return item;
							})
							.unique();
					...
					if (config.static.js) {
						hotPathJs = Object.keys(config.static.js)
							.filter(function(item) {
								return item;
							})
							.unique();
					...
					var getRealPathJs = function(item) {
						return path.resolve(
							cfg.cfgPath,
							'js',
							item.replace(/externals./, cfg.scriptLibPath + path.sep)
						);
					};
					hotPathLess.forEach(function(v, k) {
						var lessfiles = config.static.css[v]
							.filter(function(item) {
								return item;
							})
							.unique()
					...
					hotPathJs.forEach(function(v, k) {
						var jsfiles = config.static.js[v]
							.filter(function(item) {
								return item;
							})
							.unique()
...
var m = {
	...
	getConfig: function() {
		...
		var paths = [
			path.join(cfg.workPath, cfg.cfgname),
			path.join(path.dirname(cfg.workPath), cfg.cfgname),
			path.join(cfg.workPath, 'static', cfg.cfgname),
		];
		var configFile;
		for (var i in paths) {
			var f = paths[i];
...
var service = {
	gitpull: function(cfg, successCallback, errorCallback) {
		if (!service.delay) {
			...
			service.delay = setTimeout(function() {
				service.delay = null;
			}, 15000);
...
(function(global) {
	...
	global.cfg = {
		...
		error404:
			"<title>Error..</title><center><span style='font-size:300px;color:gray;font-family:Monaco,arial'>404...</span></center>",
	};
	if (args.indexOf('-v') >= 0) {
		...
		args.forEach(function(item, index) {
			...
			if (less.length == 2 && less[1]) {
				...
				require('child_process').exec('git clone ' + clone[1] + ' && cd ' + cwd, function(
					error,
					stdout,
					stderr
				) {
